...
function App() {
  ...
  const [visitManagers, setVisitManagers] = useState([""]);
  const [items, setItems] = useState(Object.fromEntries(itemList.map((item) => [item, 0])));

  const [scores, setScores] = useState(() => {
    const savedScores = localStorage.getItem("cleanmopScores");
    return savedScores ? JSON.parse(savedScores) : {};
  });

  useEffect(() => localStorage.setItem("cleanmopWorks", JSON.stringify(works)), [works]);
  useEffect(() => localStorage.setItem("cleanmopScores", JSON.stringify(scores)), [scores]);
  ...
  const saveWork = () => {
    ...
    if (visitManagers.some((m) => !m)) {
  alert("방문매니저를 모두 선택하세요.");
  return;
}
  ...
  const startEditWork = (work) => {
    ...
    setTeamCount(managers.length);
    setVisitManagers(managers);
    setItems(work.items);
    setPage("workAdd");
  };
  ...
  const calculateCodeNameStats = () => {
  const result = {};

  monthlyWorks.forEach((work) => {
    const name = work.codeName || "없음";
    ...
    result[name].workCount += 1;

    result[name].companies[work.company] =
      (result[name].companies[work.company] || 0) + 1;

    Object.entries(work.items).forEach(([itemName, count]) => {
      if (count > 0) {
        result[name].itemCount += count;
        result[name].items[itemName] =
          (result[name].items[itemName] || 0) + count;
      }
    });
  });
  ...
  const saveScore = (manager, field, value) => {
    const key = `${selectedMonth}-${manager}`;
    setScores({
      ...scores,
      [key]: {
        voc: scores[key]?.voc || 0,
        score: scores[key]?.score || "",
        memo: scores[key]?.memo || "",
        [field]: value,
      },
    });
  };
    ...
    <div style={{ padding: "20px" }}>
      ...
      {page === "home" && (
        <>
          ...
          <button onClick={() => setPage("scoreManage")}>VOC / 기사점수 관리</button><br /><br />
          <button onClick={() => setPage("companyStats")}>업체별 실적</button><br /><br />
          <button onClick={() => setPage("codeNameStats")}>코드명 실적</button><br /><br />
        </>
      )}
  ...
  {    length: workType === "solo" ? 1 : teamCount
            ...
            <div key={index}>
              ...
              <select
                ...
                onChange={(e) => {
                  const newManagers = [...visitManagers];
                  newManagers[index] = e.target.value;
                  setVisitManagers(newManagers);
                }}
              >
      ...
      {page === "scoreManage" && (
        <>
          ...
          {managerList.filter((name) => name !== "없음").map((manager) => {
            const key = `${selectedMonth}-${manager}`;
            const score = scores[key] || {};
            return (
              <div key={manager} style={{ border: "1px solid #ccc", padding: "10px", margin: "10px 0" }}>
                <h3>{manager}</h3>

                <label>VOC 건수</label><br />
                <input type="number" value={score.voc || 0} onChange={(e) => saveScore(manager, "voc", Number(e.target.value))} /><br /><br />

                <label>기사점수</label><br />
                <input type="number" value={score.score || ""} onChange={(e) => saveScore(manager, "score", e.target.value)} /><br /><br />

                <label>메모</label><br />
                <textarea value={score.memo || ""} onChange={(e) => saveScore(manager, "memo", e.target.value)} />
              </div>
            );
          })}
  ...
  <>
    ...
    {calculateCodeNameStats().length === 0 && (
      <p>코드명 실적이 없습니다.</p>
    )}

    {calculateCodeNameStats().map((data, index) => (
      <div
        key={data.codeName}
        style={{
